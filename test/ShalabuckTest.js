// Based off https://github.com/droxey/rainbowcoin/blob/master/test/RainbowCoinTest.js
...
contract('Shalabuck', async function (accounts) {
  ...
  before(done => {
    (async () => {
      try {
        ...
        let token = await Shalabuck.new("Shalabuck", "RGB", metadata.address);
  ...
  describe('Shalabuck.sol', function () {
    it('Should pass canary test', async () => {
      assert(
        true === true,
        'this is true'
      )
